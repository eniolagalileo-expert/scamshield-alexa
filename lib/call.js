...
const NEVER_CALL = [
  { re: /microsoft|windows|apple|icloud|geek squad|tech support|norton|mcafee/i, flag: "Unsolicited tech-support call", why: "Microsoft, Apple and antivirus companies never call you out of the blue about viruses or problems with your computer.", score: 9 },
  { re: /\birs\b|tax/i, flag: "Unexpected call from the IRS", why: "The IRS contacts you by mail first, and never demands immediate payment or threatens arrest by phone.", score: 5 },
  { re: /social security|\bssa\b/i, flag: "Unexpected Social Security call", why: "Social Security won't threaten you, suspend your number, or demand payment by phone.", score: 5 },
];

// Ask the most relevant question first, based on who is calling and what they said.
const CONTEXT_ORDER = [
  { re: /microsoft|windows|apple|computer|virus|hack|tech|laptop|internet/i, first: ["asked_for_remote_access", "asked_for_unusual_payment"] },
  { re: /bank|card|account|fraud|paypal|venmo|zelle|chase|wells/i, first: ["asked_for_code", "asked_to_move_money"] },
  { re: /irs|tax|police|sheriff|court|jury|warrant|social security|government|immigration/i, first: ["asked_for_unusual_payment", "threatened_or_rushed"] },
  { re: /grand|son|daughter|nephew|niece|family|accident|jail|hospital/i, first: ["asked_for_unusual_payment", "told_to_keep_secret"] },
];
...
export function checkPhoneCall({ caller_claims_to_be: claim = "", what_they_want: want = "", caller_number: number = "", answers = {} } = {}) {
  ...
  const inferred = {
    asked_for_code: /\b(code|otp|pin|verification|password|6 digits?)\b/.test(text) || undefined,
    asked_for_unusual_payment: /\b(gift ?cards?|bitcoin|crypto|wire|western union|zelle|cash ?app|venmo|prepaid)\b/.test(text) || undefined,
    ...
    threatened_or_rushed: /\b(arrest|warrant|police|fine|lawsuit|deport|shut ?off|disconnect|suspend|immediately|right now)\b/.test(text) || undefined,
  };
