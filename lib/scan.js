...
const SIGNALS = [
  ...
  },
  {
    ...
    re: /\b(urgent(ly)?|immediately|right away|asap|within \d+ (hours?|days?)|today|final notice|last chance|act (now|fast)|expires?|suspend(ed|ion)?|locked|temporarily (locked|suspended)|permanently deleted|será devuelto|en las próximas \d+ horas|urgente|hoy mismo|inmediatamente|bloquead[ao]|suspendid[ao]|segera|तुरंत|आज)\b/gi,
  ...
  },
  {
    ...
    re: /\b(if|in case) ((you )?(did ?n[o']t|didn'?t|have not|haven'?t) (place|make|authori[sz]e|request|recogni[sz]e)|(this|it) (was ?n[o']t|was not|is ?n[o']t|is not) you)[^.!?]{0,60}\b(call|contact|dial)\b[^.!?]{0,60}(\+?\d[\d\s().-]{7,}\d)/gi,
...
const REASSURANCE = [
  { re: /\b(don'?t|do not|never) share (this|the) code|will never ask (you )?for (it|your)|\bno (lo|la) comparta\b|\bnunca (le )?(pediremos|solicitaremos)\b|\bno comparta este c[oó]digo\b/i, weight: -4 },
...
export function scanMessage(text, { country, speak } = {}) {
  ...
  return {
    ...
    speech,
    language,
    // Strongest signals first, so any list or summary leads with what matters most.
    red_flags: ranked.map(({ flag, evidence, why }) => ({ flag, evidence, ...(why ? { why } : {}) })),
    links,
    phones,
