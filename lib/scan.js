...
const SIGNALS = [
  ...
  },
  {
    ...
    re: /\b(send|pay|transfer|deposit)\b[^.!?]{0,40}\b(zelle|cash ?app|venmo|paypal me)\b|\b(via|by|through|with) (zelle|cash ?app|venmo)\b|\b(zelle|cash ?app|venmo) to \$/gi,
  ...
  },
  {
    ...
    re: /\b(one[- ]time (code|password)|otp|verification code|6-digit code|password|pin number|ssn|social security number|card number|cvv|bank (login|details)|log ?in to verify|verify your (identity|account)|update your (billing|kyc|details)|kyc)\b|OTP|साझा करें|contraseña/gi,
  ...
  {
    id: "threat", weight: 3,
    re: /\b(arrest|warrant|legal action|lawsuit|police|penalty|suspension of your licen[cs]e|court|deported?|बंद कर दिया जाएगा)\b/gi,
...
export function scanMessage(text, { country } = {}) {
  ...
  for (const l of links) {
    if (l.risk === "high") {
      score += 5;
      flags.push({ weight: 5, flag: "Fake or disguised link", evidence: l.url, why: l.flags[l.flags.length - 1], say: `the link ${l.flags.some((f) => /look-alike|Mentions/.test(f)) ? "pretends to be a real company but goes to a different website" : "is disguised"}` });
    } else if (l.risk === "medium") {
      score += 1;
      flags.push({ weight: 1, flag: "Unusual link", evidence: l.url, why: l.flags[0], say: "the link looks unusual" });
    }
  }
  for (const p of phones) {
    if (p.risk === "high") {
      score += 3;
      flags.push({ weight: 3, flag: "Suspicious phone number", evidence: p.number, why: p.flags[0], say: "the phone number doesn't match who they claim to be" });
    } else if (p.risk === "medium") {
      score += 1;
      flags.push({ weight: 1, flag: "Unusual phone number", evidence: p.number, why: p.flags[0], say: "the phone number is unusual" });
    }
  }
  ...
  const speech = [opening, reasons.length ? `${titleCase(joinReasons(reasons))}.` : "", action].filter(Boolean).join(" ");
