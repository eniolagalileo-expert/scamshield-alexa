...
// People reading a text aloud say "usps dot com dash track dot top slash pkg". Turn spoken web
// addresses back into real ones so they can be checked. Only rewrites spans that end in a known
// web ending ("dot com", "dot top", ...), so ordinary sentences are left alone.
...
const SPOKEN_URL = new RegExp(
  ...
  `[a-z0-9]+(?:\\s+(?:dash|hyphen|dot)\\s+[a-z0-9]+)*` +                                           // name parts
  `\\s+dot\\s+(?:${SPOKEN_TLDS})\\b` +                                                         // "dot com"
  `(?:\\s+slash\\s+[a-z0-9]+(?:\\s+(?:dash|hyphen|dot)\\s+[a-z0-9]+)*)*`,                      // "/path"
  "gi"
);

export function normalizeSpokenLinks(text) {
  return String(text || "").replace(SPOKEN_URL, (m) => m
    .replace(/\bh\s*t\s*t\s*p\s*(s?)\s*(?:colon|:)\s*(?:slash\s*slash|\/\/)\s*/i, "http$1://")
    .replace(/\bw\s*w\s*w\s+dot\s+/i, "www.")
