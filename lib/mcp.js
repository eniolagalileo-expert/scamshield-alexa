...
import { askSecondOpinion, combine, supportsSampling } from "./second-opinion.js";
import { cleanText, officialAnswer } from "./official.js";
import { SITUATIONS, scamRecovery } from "./recovery.js";
...
If they ask what scams are going around, use scam_briefing. If they want to practice or learn, use practice_quiz (read the message, take their guess, then call it with action "answer"). After a scam verdict, offer warn_family so they can warn a relative.
If they say they already paid, clicked, shared a code or let someone into their device, use scam_recovery right away and read the first steps.
...
export function createServer() {
  ...
  server.registerTool("check_message", {
    ...
    description: "Analyze a text, email, DM or call transcript for scam tactics: disguised links, suspicious numbers, pressure, threats, untraceable payment requests, requests for codes, and more. Returns a verdict, a short spoken answer, the red flags with evidence, and next steps. Works offline and instantly. If the client supports MCP sampling, borderline messages also get a second opinion from the client's own model (it can only raise caution).",
    inputSchema: {
    ...
    outputSchema: {
      ...
      second_opinion: z.object({ verdict: z.string(), reason: z.string(), used: z.boolean() }).optional()
        .describe("Present when the client supports MCP sampling: its model's verdict; used=true if it raised the caution level"),
    },
    annotations: READ_ONLY,
  }, async ({ message, country }) => {
    let r = scanMessage(message, { country });
    // If the rules didn't call it a scam and the client can run a model for us, get a second opinion.
    if (r.verdict !== "scam" && supportsSampling(server)) {
      r = combine(r, await askSecondOpinion(server, message, r));
    }
    return reply(r.speech, {
      ...
      ...(r.second_opinion ? { second_opinion: r.second_opinion } : {}),
    });
  });
  ...
  server.registerTool("verify_with_official_source", {
    ...
    inputSchema: {
      ...
      topic: z.string().max(60).optional().describe("Optional short scam topic to search for, e.g. 'redelivery fee' or 'account locked'"),
    },
    annotations: WEB,
  }, async ({ company, question, topic: givenTopic }) => {
    ...
    // The question's own words (minus the company name) make a focused search topic.
    const topic = givenTopic || question.replace(new RegExp(`\\b(${company}|${name})\\b`, "gi"), "").replace(/\b(does|do|is|are|ever|about|the|a|an|text|texts|send|sends)\b|[?.!]/gi, " ").replace(/\s+/g, " ").trim().slice(0, 60);
    const found = await officialAnswer({ company, question, topic }, { webSearch, extractPages, brands: BRANDS, name });
    if (!found) return reply(`I couldn't find an official ${name} page about scams.`, { company, results: [] });
    const { page, best, clear, text } = found;
    // Two sentences if they're short enough to listen to comfortably, otherwise just the best one.
    // A clear statement from the company itself ("PayPal will never ask…") is the best thing to say out loud.
    const spoken = clear || (best.join(" ").length <= 240 ? best.join(" ") : best[0]);
    const speech = spoken
      ? `${name.replace(/^the /, "The ")}'s official website says: ${spoken}`
      : `I found ${name}'s official page about scams. It's on your screen.`;
    return reply(speech, { company, official_url: page.url, title: page.title, clear_statement: clear || null, key_points: best, excerpt: cleanText(text).slice(0, 700) });
  });
  ...
  server.registerTool("scam_recovery", {
    title: "I already paid, clicked or shared something",
    description: "Step-by-step help after someone has already been scammed or thinks they might have been: paid with a gift card, wire, payment app, card or crypto; shared a code or password; let someone into their computer; gave out personal details; or only clicked a link. Most urgent steps first. If what happened isn't clear, it returns status need_details and a question to ask.",
    inputSchema: {
      what_happened: z.string().max(600).optional().describe("What the person says happened, in their words"),
      situations: z.array(z.enum(Object.keys(SITUATIONS))).max(3).optional().describe("Known situations, if you already know them"),
      language: z.enum(["en", "es"]).optional().describe("Answer language (default en)"),
    },
    annotations: READ_ONLY,
  }, async (args) => {
    const r = scamRecovery(args);
  ...
  server.registerTool("practice_quiz", {
    title: "Practice spotting scams",
    description: "A short voice quiz: action 'next' returns a message to read aloud; after the person guesses 'scam' or 'real', call action 'answer' with the id and their guess to get the explanation. Pass ids already used in `seen` to avoid repeats.",
    inputSchema: {
      action: z.enum(["next", "answer"]).describe("'next' for a new message, 'answer' to check a guess"),
      id: z.string().max(20).optional().describe("The message id from 'next' (required for 'answer')"),
      guess: z.string().max(60).optional().describe("What the person said, e.g. 'scam' or 'it's real'"),
      seen: z.array(z.string().max(20)).max(50).optional().describe("Ids already asked in this session"),
    },
    annotations: READ_ONLY,
  }, async (args) => {
    const r = practiceQuiz(args);
    return r.error ? { content: [{ type: "text", text: r.error }], isError: true } : reply(r.speech, r);
  });
