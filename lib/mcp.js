...
export function createServer() {
  ...
  server.registerTool("verify_with_official_source", {
    ...
    const topic = question.replace(new RegExp(`\\b(${company}|${name})\\b`, "gi"), "").replace(/\b(does|do|is|are|ever|about|the|a|an|text|texts|send|sends)\b|[?.!]/gi, " ").replace(/\s+/g, " ").trim().slice(0, 60);
