...
const SPOKEN_QUOTE = /\b(never|will not|won'?t|does not|doesn'?t|do not|don'?t)\b[^.]{0,80}\b(ask|request|send|call|text|e-?mail|charge|contact|require)/i;
...
async function officialQuote(brand, topic = "", budgetMs = 5000) {
  ...
  const work = officialAnswer(
    ...
    const own = new RegExp(`\\b(the )?${name.replace(/^the /i, "")}\\b`, "i");
    const clear = (found?.best || [])
      ...
      .find((s) => SPOKEN_QUOTE.test(s) && (own.test(s) || /\b(we|our)\b/i.test(s)) && !/footnote|you might be thinking|same page link/i.test(s) && s.length <= 220);
    if (!clear) return "";
